...
function initContactForm() {
  const form = document.querySelector('#contact-form, form[data-contact-form]');
  if (!form || form.dataset.formspreeInitialized === 'true') return;

  form.dataset.formspreeInitialized = 'true';

  const endpoint = 'https://formspree.io/f/xqpzbdyq';
  const submitButton = form.querySelector('button[type="submit"], [data-fs-submit-btn]');
  const success = form.querySelector('[data-fs-success]');
  const error = form.querySelector('[data-fs-error]');
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        form.reset();
        showMessage(success, 'Thanks! Your inquiry has been sent successfully.');
      } else {
      ...
      showMessage(error, 'Unable to send your inquiry right now. Please try again or email Paige directly at perickson0222@gmail.com.');
    } finally {
      if (submitButton) {
